...
class Controller():
    ...
    def deleteuser(self,id):
        query = "delete from pet_users where id ='"+id+"'"
        self.db.insert(query)
        return 1


    def deletepost(self,number):
        query = "delete from pet_posts where no ='"+number+"'"
        self.db.insert(query)
        return 1


    def get_all_report_list(self):
        query="select * from pet_reports order by id desc"
        rlist=self.db.select_all(query)
        all_list=list()
        for i in rlist:
            temp=self.report.makeDicByReport(self.report.create_from_dbdata(i))
    ...
    def get_one_report(self,num):
        query = "select pet_reports.id, title, type, uid, time, content, answer from pet_reports join pet_report_type on tid=pet_report_type.id where pet_reports.id=%d"
        report=self.db.select_one(query%num)

        return report


    def save_answer(self,num,data):
        query="update pet_reports set answer='%s' where id=%d"
        self.db.insert(query%(data['answer'],num))
